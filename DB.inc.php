...
class DB
{
	...
	private function __construct()
	{
	  ...
	  $connStr = 'pgsql:host=localhost port=5432 dbname=mydb'; // A MODIFIER ! 
	  try
	  {
	  // Connexion à la base
	  	  $this->connect = new PDO($connStr, 'csdc', 'CS!DC@UniTwin'); //A MODIFIER !
	...
	*************************************************************************/
	...
	public function insert($i="", $n="", $p="", $r="", $g="",$m="", $dC="")
	{
		$requete = 'insert into client values(?,?,?,?,?,?,?)' ;
		$tparam  = array($i, $n, $p, $r, $g,$m ,$dC)       ;
		 return $this->execMaj($requete,$tparam)                                ;
	}
	
	public function getIpAddress()
	{
		$requete = 'SELECT client_ip_address FROM client';
		return $this->execQuery($requete, null, 'Utilisateur');
	}
